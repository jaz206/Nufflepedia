...
export default async function TablasPage() {
  ...
  const sectionTitle = "text-lg font-semibold pb-2 mb-3";
  const sectionTitleStyle = { borderBottom: "1px solid var(--border)", color: "var(--gold)" } as const;
          ...
          <ul className="space-y-2 text-sm">
            {playerStates.map((s) => (
              <li key={s.id} className="flex gap-3">
                ...
                <span style={{ color: "var(--ink-2)" }}>{s.description}</span>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ul className="space-y-2 text-sm">
            {weather.map((w) => (
              <li key={w.id} className="flex gap-3">
                <span className="w-12 shrink-0 font-mono" style={{ color: "var(--ink-3)" }}>
          ...
          <ul className="space-y-2 text-sm">
            {injury.map((i) => (
              <li key={i.id} className="flex gap-3">
                <span className="w-16 shrink-0 font-mono" style={{ color: "var(--ink-3)" }}>
          ...
          <ul className="space-y-2 text-sm">
            {inducements.map((ind) => (
              <li key={ind.id} className="flex gap-3">
                <span className="w-28 shrink-0 font-mono text-xs" style={{ color: "var(--ink-3)" }}>
                  0-{ind.maxCount} · {ind.cost !== null ? `${ind.cost.toLocaleString("es-ES")} MO` : "variable"}
                </span>
                <span>
                  <strong>{ind.name}</strong>
                  {ind.restriction && (
                    <span style={{ color: "var(--ink-3)" }}> ({ind.restriction})</span>
                  )}{" "}
                  — {ind.effect}
                </span>
              </li>
